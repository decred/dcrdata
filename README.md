...
**Beware:** For the v5 dcrdata module, the executable generated by `go build`
may be named "v5" instead of "dcrdata". This is a
[known issue in the `go` tool](https://github.com/golang/go/issues/27283) that
was resolved in Go 1.12.5.
...
the `/api` path prefix. The `types` package refers to `api/types`.
...
| Transaction count    | `/block/best/tx/count`               | `types.BlockTransactionCounts`        |
...
| Transaction count     | `/block/X/tx/count`   | `types.BlockTransactionCounts`        |
...
| Transaction count    | `/block/hash/H/tx/count`   | `types.BlockTransactionCounts`        |
...
| Stake submission addresses of all tickets in the pool as an array of strings                   | `/stake/pool/full/addrs`                              | `[]string`                  |
...
| Stake submission addresses of all tickets in the pool at block height _or_ hash `H`            | `/stake/pool/b/H/full/addrs`                          | `[]string`                  |
| Pool info for block range `[X,Y] (X <= Y)`                                                     | `/stake/pool/r/X/Y?arrays=[true\|false]`<sup>\*</sup> | `[]types.TicketPoolInfo` |

| Missed and Expired Tickets                              | Path                 | Type                      |
| ------------------------------------------------------- | -------------------- | ------------------------- |
| Missed and expired tickets sorted by revocation status  | `/stake/powerless`   | `types.PowerlessTickets`  |
...
| Mempool                                           | Path                      | Type                            |
| ------------------------------------------------- | ------------------------- | ------------------------------- |
| Ticket fee rate summary                           | `/mempool/sstx`           | `types.MempoolTicketFeeInfo` |
| Ticket fee rate list (all)                        | `/mempool/sstx/fees`      | `types.MempoolTicketFees`    |
| Ticket fee rate list (N highest)                  | `/mempool/sstx/fees/N`    | `types.MempoolTicketFees`    |
| Detailed ticket list (fee, hash, size, age, etc.) | `/mempool/sstx/details`   | `types.MempoolTicketDetails` |
| Detailed ticket list (N highest fee rates)        | `/mempool/sstx/details/N` | `types.MempoolTicketDetails` |
...
| Bottom line health check (HTTP 503 if not healthy)  | `/status/happy`  | `types.Happy`      |
