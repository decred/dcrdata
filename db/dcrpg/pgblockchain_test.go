...
func TestIncrementalChartsQueries(t *testing.T) {
	...
	xvalArray := map[Charts]string{
		...
		HashRate:        "Time",
		POWDifficulty:   "Time",
		TicketByWindows: "Height",
		TicketPrice:     "Time",
		TicketsByBlocks: "Height",
		TicketSpendT:    "Height",
		TxPerBlock:      "Height",
		TxPerDay:        "Time",
	}
	...
	for chartType, xValName := range xvalArray {
		...
		t.Run("check_duplicates_n_ordering_for_"+chartType, func(t *testing.T) {
			val := reflect.ValueOf(data).Elem()
			for i := 0; i < val.NumField(); i++ {
				if xValName == val.Type().Field(i).Name {
					switch strings.Lower(xValName) {
					case "height":
						...
						if len(m) != len(heightsCopy) {
							t.Fatalf("expected x-axis data for chart %s to have no duplicates but found %d",
								len(heightsCopy)-len(m))
						}

					case "time":
						// All Time dataset is an array of type time.Time
						timestamp := val.Field(i).Interface().([]time.Time)
						timestampCopy := make([]int64, len(timestamp))
						copy(timestampCopy, timestamp)

						// Sort and check for ascending order.
						sort.Slice(
							timestampCopy,
							func(i, j int) bool { return timestampCopy[i].date.Before(timestampCopy[j].date) },
						)
